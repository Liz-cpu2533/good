...
async def ark_worker(model_name, client, model_id, worker_id, task_queue, is_force_think=False):
    ...
    for i, data in enumerate(task_queue):
        ...
        final_prompt = f'你现在正在参加期货行业的从业资格考试,你已经知道答案：\n{data["原答案"]}\n请你回答问题：{data["问题"]}\n要求：回答尽量在五句话左右'
...
async def process_csv(model_name, model_id, input_path, output_path, concurrent=5, is_force_think=False):
    ...
    for i in range(len(dataset) // batchsize):
        # 分发任务
        results = await ark_batch_query(model_name, model_id, dataset[i * batchsize:(i + 1) * batchsize], concurrent,
                                        is_force_think)
